...
public class PortalController(AppDb db, IOrderRuleService rules, IFulfillmentGateway fulfillment) : ControllerBase
{
    ...
    public async Task<object> Products([FromQuery] string? criterion = "Product Name", [FromQuery] string? q = "", [FromQuery] string? query = null, [FromQuery] string? shipToAccountId = null, [FromQuery] bool favorites = false)
    {
        q = Normalize(query ?? q ?? "");
        var normalizedQuery = SearchToken(q);
        var criterionKey = Criterion(criterion);
        var all = await db.Products.ToListAsync();
        string Field(Product p) => criterionKey switch { "itemNumber" => p.ItemNumber, "activeIngredient" => p.ActiveIngredients, "productCategory" => p.Category, "gtin" => p.Gtin, "packageSize" => p.PackageSize, "supplier" => p.Supplier, "productLine" => p.ProductLine, _ => p.Name };
        bool Matches(string value) => value.Contains(q, StringComparison.OrdinalIgnoreCase) || SearchToken(value).Contains(normalizedQuery, StringComparison.OrdinalIgnoreCase);
        bool ProductMatches(Product p) => criterionKey == "all"
          ? Matches(p.Name) || Matches(p.ItemNumber) || Matches(p.ActiveIngredients)
          : Matches(Field(p));
        return all
          .Where(p => (!favorites || p.Favorite) && (q.Length == 0 || ProductMatches(p)))
          .OrderBy(p => criterionKey == "all" ? Math.Min(Rank(p.Name, q), Math.Min(Rank(p.ItemNumber, q), Rank(p.ActiveIngredients, q))) : Rank(Field(p), q))
          .ThenBy(p => p.Name);
    }
    [HttpGet("products/by-name")] public Task<object> ProductsByName([FromQuery] string? q = "") => Products("Product Name", q);
    [HttpGet("products/by-active-ingredient")] public Task<object> ProductsByActiveIngredient([FromQuery] string? q = "") => Products("Active Ingredient", q);
    [HttpGet("products/by-item-number")] public Task<object> ProductsByItemNumber([FromQuery] string? q = "") => Products("Item Number", q);
    ...
    static string Criterion(string? value) => value?.Replace(" ", "").Replace("/", "").ToLowerInvariant() switch { "all" => "all", "itemnumber" => "itemNumber", "activeingredient" => "activeIngredient", "productcategory" => "productCategory", "gtin" => "gtin", "packagesize" => "packageSize", "vendorsupplier" or "supplier" => "supplier", "productline" => "productLine", _ => "productName" };
